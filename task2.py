def binary_search_with_upper_bound(arr, x):
    """
    Здійснює двійковий пошук елемента 'x' у відсортованому списку arr.

    Параметри:
    arr (list): Відсортований список, у якому здійснюється пошук.
    x (float): Елемент, який потрібно знайти.

    Повертає:
    tuple: Кортеж, де першим елементом є кількість ітерацій,
           а другим — найменший елемент, який є більшим або рівним 'x'.
    """
